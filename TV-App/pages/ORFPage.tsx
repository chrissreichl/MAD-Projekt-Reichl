    ...
    return (
      <>
        <Card onPress={this.props.onCardClicked}>
          <Card.Content>
            <Paragraph>{this.props.subject}</Paragraph>
            <Title>{this.props.headline}</Title>
            {this.props.shortStory != "" && <Paragraph style={{ marginBottom: 10 }}>{this.props.shortStory}</Paragraph>}
          </Card.Content>
      ...
      fetch(pageUrl)
        ...
        .then((response) => {
          ...
          try {//try to find shortStory in HTML-Page
            if (root.querySelector('.lead') != null) { //orf News und Sport
              var shortStory = root.querySelector('.lead').text.trim(); //.trim() is needed, because in the string before the characters are unnecessary spaces 
              //console.log(shortStory);
            } else {
              var shortStory = root.querySelector('.paragraph').querySelector('p').text;
              //console.log(shortStory);
            }
          ...
          try {//try to find img-Url in HTML-Page
            var imageUrl = root.querySelector('.imagebox').querySelector('img').rawAttrs;
